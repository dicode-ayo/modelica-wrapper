...
| `change` | `{ layout, staleBase }` | User committed a layout change (move/resize/rotate/draw/delete). The whole layout, not a diff. `staleBase` is true when the webview refused or discarded a `layout` push from the host since the last one it applied (a gesture in flight, or a commit already queued) — the host then treats a component missing from `layout` as one it never told the webview about, not one the user deleted. |
