...
| `change` | `{ layout, staleBase }` | User committed a layout change (move/resize/rotate/draw/delete). The whole layout, not a diff. `staleBase` is true when the webview refused or discarded a `layout` push from the host since the last one it applied (a gesture in flight, or a commit already queued) — the host then drops the edits it can't trust from such a report (see `isTrustedOnStaleBase` in `diff-layout.ts`) rather than reading them off `layout`'s difference from OMC's real state. Trust is mostly per edit kind, but a `connectionAdded` from an ambiguous vector-port re-index group (2+ connections sharing a base — a cascade or swap, not a clean 1:1 rename) is distrusted too: it can complete a rename the stale report never fully saw and duplicate a connection rather than just drop the edit (issue #503). |
