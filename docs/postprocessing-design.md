...
| `status` | `{ message, error? }` | Surface a read/parse error, or a `ResultViewDocument` write failure (including a backfill write that failed to persist). |
...
| **6** | polish | rename/remove result and the missing-file chip are wired end-to-end (issue #465): `removeResult`/`renameResult` apply as `WorkspaceEdit`s, and the provider computes + sends `missingResults` on refresh (recomputed on `ready` / document edits — a filesystem-only change while the view stays open, e.g. deleting or re-creating a `.mat`, is issue #486). Still open: theme-change refresh, resize, empty states. | `result-doc`/`result-cache`/`result-view-provider` unit tests cover remove/rename and the missing-file scan. |
