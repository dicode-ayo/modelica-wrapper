...
A pnpm workspace ([`pnpm-workspace.yaml`](pnpm-workspace.yaml)) of five packages:
...
`result-ui`; `diagram-ui` → `diagram-svg` / `omc-client`; `diagram-svg` →
`omc-client`. `result-ui` and `omc-client` depend on nothing in the workspace.
