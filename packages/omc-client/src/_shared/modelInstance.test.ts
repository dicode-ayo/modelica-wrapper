...
describe("PrefixesSchema.replaceable", () => {
  ...
  it(
    "degrades an unrepresentable modifier tree to undefined rather than " +
      "rejecting the whole instance — a redeclare's $value can carry a " +
      "scodeElement whose dims/annotation are arrays, which Modifier has no " +
      "branch for",
    () => {
      const result = PrefixesSchema.safeParse({
        replaceable: {
