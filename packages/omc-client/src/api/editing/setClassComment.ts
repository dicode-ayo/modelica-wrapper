...
export const SetClassCommentInputSchema = z.strictObject({
  ...
  filename: z
    .string()
    .describe(
      "New comment text. Misnamed `filename` in OMC docs but actually the comment string; the wrapper preserves the doc name verbatim per the package convention.",
    ),
});
...
export type SetClassCommentOutput = z.infer<typeof SetClassCommentOutputSchema>;
...
export async function setClassComment(
  ...
  const raw = await ctx.call(
    `setClassComment(${bareName(input.typeName)}, ${quote(input.filename)})`,
  );
  return parseOutput(
