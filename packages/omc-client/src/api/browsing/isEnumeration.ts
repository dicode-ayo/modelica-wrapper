...
export const IsEnumerationDescription =
  "Check whether the given class is an enumeration type.";

export async function isEnumeration(
  ctx: CallContext,
  input: IsEnumerationInput,
): Promise<IsEnumerationOutput> {
  const raw = await ctx.call(`isEnumeration(${bareName(input.typeName)})`);
  return parseOutput(
