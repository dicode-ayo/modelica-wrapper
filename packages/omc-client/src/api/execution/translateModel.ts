...
export const TranslateModelDescription =
  "Translate a Modelica model into C code without building it.";

export async function translateModel(
  ctx: CallContext,
  input: TranslateModelInput,
): Promise<TranslateModelOutput> {
  const raw = await ctx.call(`translateModel(${bareName(input.typeName)})`);
  return parseOutput(
