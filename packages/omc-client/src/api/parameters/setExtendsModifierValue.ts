...
export const SetExtendsModifierValueInputSchema = z.strictObject({
...
export async function setExtendsModifierValue(
  ...
  const raw = await ctx.call(
    `setExtendsModifierValue(${bareName(input.typeName)}, ${bareName(input.extendsBase)}, ${bareName(input.modifier)}, ${codeArg})`,
  );
  return parseOutput(
