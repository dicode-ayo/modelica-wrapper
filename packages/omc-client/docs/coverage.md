...
| **Total verified** | **136** | **151** | **90%** ✅. Of the 15 unverified: 3 ⛔ (`createClass`, `createSubClass` — genuinely missing on the pin; `save` — present but deprecated/unreliable), 1 🟡 OMC-side bug (`setElementAnnotation`), 2 🟡 inherited-class map annotations, 3 🟡 extends-modifier readers/writer (need fixture), 6 🟡 network-only package-manager calls. **Audit also identified ~40 documented OMC functions in scope but not yet wrapped — see the 100% coverage epic.** |
