...
**Pinned OMC version:** `1.27.1` (see [`../src/version.ts`](../src/version.ts)). Bumped from `1.27.0` on 2026-09-15: 1.27.1 is a maintenance patch that leaves the scripting API untouched — `getClassNames(OpenModelica.Scripting, recursive=true)` returns the same 481 symbols on both, and `getModelInstance` over three MSL models (2.1 MB of JSON) is byte-identical, as are `getClassInformation` (including the 18-field not-found shape), `getComponentsTest`, `getSimulationOptions`, `getElementAnnotation`, `getNthConnection` and `getInheritedClasses`. Both suites pass against a live 1.27.1 container (omc-client 779 tests, extension 1392 tests) and the drift probe returns its baseline verdicts unchanged. The compiler-side fixes in the release (FMU directional derivatives, `spatialDistribution()`) sit outside the wrapped call surface; the rest is OMEdit. This was an integration-suite re-probe rather than the full per-function §3 audit walk. Previously bumped from `1.26.7` on 2026-07-22: both integration suites (omc-client `test/integration.test.ts`, 62 tests, and the extension's `*.integration.test.ts`, 40 tests) pass against 1.27.0. The only shape drift was `getClassInformation`, which on 1.27.0 drops the four trailing version-annotation fields for an annotation-free / not-found class (18 items, not 22) — the wrapper now reads those defensively. This was an integration-suite re-probe rather than the full per-function §3 audit walk. On 2026-05-19 a deep re-probe of the previously-⛔ wrappers revealed that 4 of them (`removeComponentModifiers`, `updateConnection`, `moveClass`, plus the newly-added `getReplaceableChoices`) were wrapper-side bugs — wrong argument shape masked by OMC's misleading "Class X not found in scope" diagnostic. See [audit.md §2.10](./audit.md) for the gotcha. After fixing the call shapes, those wrappers + state-machine mutators are now ✅ on 1.26.7. On 2026-05-20 (#35) `newModel` was found documented + working on 1.26.7 and is now wrapped (✅) as the migration path off the old `createClass`/`createSubClass` wrappers, which were genuinely ⛔ (docs 404 + `✗ not found in scope`) and have since been **removed** (2026-05-21, #80 — see the "Removed wrappers" section); `save` is now the only remaining ⛔ but on *usefulness* grounds (deprecated; persists nothing for backing-file-less classes) — not symbol-missing, and has a drift-probe counter-example. On 2026-05-20 (#38) the last 🟡 in `elements/` — `setElementAnnotation` — flipped to ✅ after the payload shape was realigned to OMEdit's canonical `$Code((<expr>))` form (the leading-`=` `$Code(=<expr>)` shape that previously shipped is silently destructive on 1.26.7). Earlier same week: added results wrappers (filter / delta / diff — `compareSimulationResults` was also added then but later removed as deprecated, see "Removed wrappers") and a heavy integration test exercising every results wrapper against a real `.mat` file.

**Last updated:** 2026-09-15 (pin bumped to 1.27.1 via an integration-suite re-probe; no wrapper drift found).
...
| `getDerivedUnits` | ✅ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.getDerivedUnits.html) — `getDerivedUnits(String baseUnit) -> String[:] derivedUnits`. The unit-dropdown partner to `convertUnits`. Verified live on 1.26.7: `getDerivedUnits("K")` → `{"degC", "degF", "degRk"}`; `getDerivedUnits("1")` → `{}`. String arg quoted (audit §2.10). Issue #63. |
...
| `writeClassGraphics` | ✅ | — | Composite editing wrapper (not a raw OMC call): reads `getIconAnnotation`/`getDiagramAnnotation`, splices the graphics list by positional `(layer, index)`, and re-emits via `addClassAnnotation`. Added 2026-06-19 for the diagram drawing write path (#186). Existing shapes are decoded and re-serialized (not echoed verbatim) because OMC emits an empty `textStyle={}` it then refuses to re-parse; the full `coordinateSystem` is carried through since `addClassAnnotation` replaces the whole layer. Live-OMC round-trip verified (add/modify/delete + coordinateSystem preservation). |
...
| `setFullDocumentationAnnotation` | ✅ | — | Composite editing wrapper (not a raw OMC call): reads `getDocumentationAnnotation` for the class's current `revisions`/`infoHeader`, then re-emits the whole `Documentation(...)` clause via `addClassAnnotation` alongside the caller's new `info` — the same read-full/write-full pattern as `writeClassGraphics`, so a write never drops a section it wasn't asked to change. `setDocumentationAnnotation` has no `infoHeader` parameter and clears it. Added 2026-07-14 (#304). Unit-tested (command composition); its two constituent calls are independently live-verified. |
...
| Editing | 23 | 23 | All ✅. **`updateConnection` rescued**: arg order + String-quoting fix. **`addTransition`/`deleteTransition` bugfix**: same String-quoting gotcha — they were silently 🟡 broken before. **Issue #37 (2026-05-20)** added `updateConnectionNames` + `updateTransition`; `updateConnectionAnnotation` is intentionally skipped because `updateConnection`'s `annotate` arg already supersedes it. **`writeClassGraphics` (#186, 2026-06-19)**: composite graphics read-modify-write over `addClassAnnotation`. **`setFullDocumentationAnnotation` (#304, 2026-07-14)**: composite Documentation-annotation read-modify-write over `addClassAnnotation`, so `__OpenModelica_infoHeader` survives a write. |
