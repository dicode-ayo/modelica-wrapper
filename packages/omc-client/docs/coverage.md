...
**Pinned OMC version:** `1.26.1` (see [`../src/version.ts`](../src/version.ts)).
**Last updated:** 2026-05-05.
**Current coverage:** 53 / 80 functions (66%).
...
| `loadString` | 🟡 | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.loadString.html) | Used by every fixture, but no dedicated assertion |
...
| `createClass` | ⛔ | 404 | Undocumented in 1.26 public scripting API; OMC reports "Class createClass not found". Wrapper kept; may exist on other OMC versions or under an internal namespace. |
| `createSubClass` | ⛔ | 404 | Same as `createClass`. |
| `renameClass` | ⛔ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.renameClass.html) | Documented (`output TypeName[:] result`) but our test setup needs `createSubClass`. Re-enable once a non-create fixture path exists. |
| `deleteClass` | ⛔ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.deleteClass.html) | Used by `disposeFixture` (best-effort). Returns null on 1.26 even when it does delete; no clean assertion path yet. |
| `copyClass` | ⛔ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.copyClass.html) | Documented (output is `result`, not `success`) but OMC 1.26.1 reports "not found" — possibly moved. |
| `moveClass` | ⛔ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.moveClass.html) | Same as `copyClass` — needs `createSubClass` for setup. |
| `moveClassToTop` | ⛔ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.moveClassToTop.html) | Same. |
| `moveClassToBottom` | ⛔ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.moveClassToBottom.html) | Same. |
...
| `save` | ⛔ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.save.html) | OMEdit-deprecated; we use Option B persistence. Wrapper exposed for completeness only. |
...
| `removeComponentModifiers` | ⛔ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.removeComponentModifiers.html) | OMC 1.26.1 reports "not found"; needs OMC-version-specific verification. |
...
| `updateConnection` | ⛔ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.updateConnection.html) | OMC 1.26.1 reports "not found"; needs version-specific verification. |
...
| `setComponentProperties` | ⛔ | [docs](https://build.openmodelica.org/Documentation/OpenModelica.Scripting.setComponentProperties.html) | Wrapper sends a different argument shape than OMC 1.26 expects. Docs declare 6 inputs (`prefixArray, variability, innerOuter, direction`), wrapper sends 4-element prefix + 3-string tuple. Re-aligning is a small refactor. |
...
| Lifecycle | 6 | 16 | 10 ⛔ — most blocked by undocumented `createClass`/`createSubClass` (needed for fixture setup of the rest) or signature drift in 1.26 |
| Parameters & modifiers | 5 | 9 | 1 ⛔ (`removeComponentModifiers`), 3 🟡 (extends-with-modifications fixture) |
| Editing | 9 | 13 | 2 ⛔ (`updateConnection`, `setComponentProperties` — both signature/symbol issues on 1.26), 2 🟡 (state-machine fixture for transitions) |
...
3. **⛔ symbol-not-found cases (`createClass`, `createSubClass`, `copyClass`, `moveClass`, `moveClassToTop`, `moveClassToBottom`, `setComponentProperties`, `updateConnection`, `removeComponentModifiers`):** these need OMC-version-specific investigation. Options:
   - Probe whether they live in an internal namespace (`OpenModelica.Scripting.Internal.*` or similar).
   - Read OMC 1.26 source (`Compiler/Script/CevalScriptBackend.mo` at the matching tag) for the canonical implementation.
   - If permanently removed: deprecate the wrapper here and document the alternative path (e.g. `loadString` for class creation).

4. **`setComponentProperties` signature drift:** docs say 6 inputs (`prefixArray, variability, innerOuter, direction`); our wrapper sends 4-element prefix + 3-string tuple. Re-align the wrapper, then re-add the test.
