...
export function registerSaveTools(server: McpServer, deps: McpToolDeps): void {
  server.registerTool(
    ...
    {
      description:
        "Write a class to its source file, creating one under the workspace when it has none and keeping the enclosing package's package.order current. Edits made by addComponent, addConnection, setElementModifierValue, setSourceCode and the shape tools change OMC's memory only — call this to persist them. On a package it saves the members too.",
