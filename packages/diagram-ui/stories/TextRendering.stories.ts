 ...
 *
 * `text-mode` reaches `<om-text>` through `textModeContext`, which is a term
 * in the primitive's rebuild key, so switching it rebuilds the text in place
 * — the scene, its renderer and the view state all survive. Zoom drives
 * `<om-scene zoom>` on that same live scene, which is the path that
 * exercises the `resolution` ramp on zoom-in.
 */
...
import type { TextMode } from "../src/primitives/text-mode.js";
...
const meta: Meta<StoryArgs> = {
  ...
  render: ({ textMode, zoom }: StoryArgs): TemplateResult => html`
    <div class="om-story">
      ...
      <p class="om-story-caption">
        ...
        <code>html</code> (<code>HTMLText</code>, browser layout via an SVG
        foreignObject).
      </p>
      <p class="om-story-caption">
      ...
      <div class="om-story-canvas-host" style="height: 560px;">
        <om-scene .zoom=${zoom} .textMode=${textMode}>
...
export const PID: Story = {
  ...
  parameters: { chromatic: { disableSnapshot: true } },
  render: ({ textMode }: StoryArgs): TemplateResult => html`
    <div class="om-story">
      <h3>
      ...
      <p class="om-story-caption">
        ...
        <code>k=1</code>, <code>J=1 kg.m2</code> or <code>Ti=0.1 s</code> — the
        small parameter captions are where the backends separate. Two-finger
