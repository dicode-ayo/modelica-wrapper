 ...
 *
 * The shape bodies use Lit's `svg` tag, not `html`: a `<path>` built in a
...
export const undoIcon = glyph(
  svg`<path d="M5 4 2 7l3 3" /><path d="M2 7h7a4 4 0 0 1 0 8H6" />`,
);

export const checkIcon = glyph(
  svg`<path d="M3 8.5 6.5 12 13 4" stroke-width="2" />`,
);

export const simulateIcon = glyph(
  svg`<path d="M5 3.5v9l7-4.5z" fill="currentColor" stroke="none" />`,
);

export const parametersIcon = glyph(
  svg`<path d="M2 5h12M2 11h12" />
...
export const rotateIcon = glyph(
  svg`<path d="M13 8a5 5 0 1 1-1.8-3.85" /><path d="M13 2.5V5h-2.5" />`,
);
...
export const rectangleIcon = glyph(
  svg`<rect x="2.5" y="4" width="11" height="8" rx="0.5" />`,
);

export const ellipseIcon = glyph(svg`<ellipse cx="8" cy="8" rx="6" ry="4" />`);

// `wa-button with-caret` renders a `wa-icon` (fetch-loaded, CSP-blocked here),
// so the dropdown trigger slots this caret instead.
export const caretIcon = glyph(svg`<path d="M4 6l4 4 4-4" />`);
